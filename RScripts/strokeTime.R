...
for(char in charNamesT)
{
  ...
  if(exists(paste(char,"_D_1",sep="")))
  {
    ...
    delT1 = c(delT1,diff(char_1[,p]))
    print(length(delT1))
    delT12 = c(delT12,diff(char_12[,p]))
    print(length(delT12))
    delD1 = c(delD1,diff(char_12[,p]-char_1[,p]))
    
    namesChar1 = c(namesChar1,paste(char,1:(length(char_1[,1])-1),sep="_"))
    ...
    
  {
    ...
    delT2 = c(delT2,diff(char_2[,p]))
    print(length(delT2))
    delT21 = c(delT21,diff(char_21[,p]))
    print(length(delT21))
    delD2 = c(delD2,diff(char_2[,p]-char_21[,p]))
    
    namesChar2 = c(namesChar2,paste(char,1:(length(char_2[,p])-1),sep="_"))
...
barplot(t(as.matrix(delV)),beside=TRUE,col=c('blue','green','red'),cex.names=0.5,main="Stroke Time")
