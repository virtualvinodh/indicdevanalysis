...
delVK = delVK[abs(delVK$K3) > 5,]
barplot(t(as.matrix(delVK)),beside=TRUE,col=c('blue','green','red'),cex.names=0.5,main="Stroke Length")

delS_K = c(delD1K,delD2K)
print(length(delK_T))
delS_K_O = delS_K
delS_K = delS_K[abs(delS_K) > 5] 
print(length(delS_K))
barplot(delS_K,main="Tamil Distance")

print("Stroke with Change")
print(length(delS_K)/length(delS_K_O))
print("Stroke without Change")
print(1-length(delS_K)/length(delS_K_O))
print("Strokes with Increase")
print(length(delS_K[delS_K > 0])/length(delS_K))
